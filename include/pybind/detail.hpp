...
namespace pybind
{
	...
	{				
		...
		PYBIND_API uint32_t find_class_info_desc_name( const char * _name );
		PYBIND_API const class_info_desc_t * get_class_info_desc_id( uint32_t _id );
		...
		PYBIND_API uint32_t get_object_type_id( PyObject * _type );
