...
namespace pybind
{
    ...
    struct method_proxy_call
    {
        static PyObject * call( kernel_interface * _kernel, P * _proxy, C * _obj, F f, PyObject * _arg )
        {
            ...
            static_assert(f_info::method == true, "[pybind] method proxy bind requires a method");
            static_assert(std::is_convertible<P *, typename f_info::class_type *>::value == true, "[pybind] method proxy class type is incompatible");
            static_assert(f_info::arity >= 1, "[pybind] method proxy bind requires an object argument");
